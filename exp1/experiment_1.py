...
path='/data1/lwt/model/all-MiniLM-L6-v2'
...
def count_similarity(sent0, sent1):

    embeddings = model.encode([sent0, sent1])
    # print(embeddings)

    emb0 = embeddings[0]
    emb1 = embeddings[1]

    cosine_score = util.cos_sim(emb0, emb1) # tensor([[0.4046]])
...
def exp1(fn):
    ...
    # 写入CSV文件
    csv_file_path = f"{fn}.csv"
    with open(csv_file_path, mode="w", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        # 写入表头
        writer.writerow(["sent1", "sent2", "similarity", "test-time", "test-level"])
        # 写入数据
        for i in range(len(sent1_ls)):
            writer.writerow([sent1_ls[i], sent2_ls[i], similarity_ls[i], test_time_ls[i], test_level_ls[i]])
...
def draw_pic_easy(alg_ls=['chatglm3-6B','chatglm-turbo','GPT-4-1106','GPT-3.5-turbo']):
    ...
    for idx,alg in enumerate(alg_ls):
        ...
        plt.plot(labels, values, marker='o',label=alg)  # 使用圆形标记
    ...
    plt.savefig('exp1_easy.pdf', format='pdf')
def draw_pic_hard(alg_ls=['chatglm3-6B','chatglm-turbo','GPT-4-1106','GPT-3.5-turbo']):
    ...
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.xlabel('Time scale')
    plt.savefig('exp1_hard.pdf', format='pdf')
# exp1('chatglm3-6B')
# exp1('chatglm-turbo')
...
# exp1('echo-m3')
# exp1('echo-28-qwen2')
# exp1('echo-m3-chatglm3')
# exp1('echo-28-qwen-epoch4')
# exp1('echo-28-qwen-epoch10')

draw_pic_easy(alg_ls=['chatglm3-6B','chatglm-turbo','GPT-4-1106','GPT-3.5-turbo','llama3-8b','echo-m3','echo-m3-chatglm3','echo-28-qwen2','echo-28-qwen-epoch4','echo-28-qwen-epoch10'])

draw_pic_hard(alg_ls=['chatglm3-6B','chatglm-turbo','GPT-4-1106','GPT-3.5-turbo','llama3-8b','echo-m3','echo-m3-chatglm3','echo-28-qwen2','echo-28-qwen-epoch4','echo-28-qwen-epoch10'])