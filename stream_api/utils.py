...
def generate_stream_chatglm3(model: PreTrainedModel, tokenizer: PreTrainedTokenizer, params: dict):
    ...
    # timeinfo='2024年5月13日星期一20点32分14秒'
    max_new_tokens = int(params.get("max_tokens", 1000))
    echo = params.get("echo", True)
    messages = process_chatglm_messages(messages, functions=functions)
    query, role = messages[-1]["content"], messages[-1]["role"]
    # print('query, role ',query, role )
    inputs = tokenizer.build_chat_input(query, timeinfo, history=messages[:-1], role=role)#
...
def process_chatglm_messages(messages, functions=None):
    ...
    for m in _messages:
        ...
        if role == "function":
            ...
            messages.append({"role": role, "metadata": metadata, "content": content})
