...
path='/home/lwt/Model/all-MiniLM-L6-v2'
...
def exp3(fn):
    ...
    for idx,one in enumerate(testd):
        ...
        while assi_idx<length_chat:
            ...
            if conv[test_idx]['test']==1:
                ...
                if test_level=='easy':
                    test_time_dict_easy+=[simi,]
                else:
                    test_time_dict_hard+=[simi,]
    ...
    with open(csv_file_path, mode="w", newline="", encoding="utf-8") as file:
        ...
        for i in range(len(sent1_ls)):
            writer.writerow([sent1_ls[i], sent2_ls[i], similarity_ls[i], test_level_ls[i]])
        
        # 写入总数据
        writer.writerow(['mean-easy'])
        writer.writerow([sum(test_time_dict_easy)/len(test_time_dict_easy)])
        writer.writerow(['mean-hard'])
        writer.writerow([sum(test_time_dict_hard)/len(test_time_dict_hard)])
...
def draw_pic_easy(alg_ls=['chatglm3-6B','chatglm-turbo','GPT-4-1106','GPT-3.5-turbo']):
    ...
    # 添加标题和轴标签
    plt.title('Time-independent Easy Episodic Memory')
    # plt.legend(loc="upper right")
    # plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.xlabel('Models')
    plt.savefig('exp3_easy.pdf', format='pdf')
...
def draw_pic_hard(alg_ls=['chatglm3-6B','chatglm-turbo','GPT-4-1106','GPT-3.5-turbo']):
    ...
    for bar in bars:
        ...
        plt.text(bar.get_x() + bar.get_width()/2, yval + 0.001, round(yval, 2), ha='center', va='bottom')
    # 添加标题和轴标签
    plt.title('Time-independent Hard Episodic Memory')
    # plt.legend(loc="upper right")
    # plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.xlabel('Models')
    plt.savefig('exp3_hard.pdf', format='pdf')
